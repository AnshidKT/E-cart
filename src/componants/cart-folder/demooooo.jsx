...
const Modal1 = ({onClose,open,formattedTotal,formattedAmnt,formattedTotalAmount,taxAmt}) => {
  ...
  const {cartItems , getTotalCartAmount, taxAmount,updateCartItemCount,cartAmount} = useContext(ShopContext)
...
const handleShareWhatsApp = () => {
  ...
  const servicesDetails = combinedCards.filter((product) => cartItems[product.id] !== 0)
    .map((product) => `\nProduct:  ${product.span_head} | Qty:  ${cartAmtPerItem[product.id]} | Price : ${product.span_body} `)
    .join('\n');
...
// Phone Number: 79946536071
  ...
  return ReactDom.createPortal(
    ...
    <div className='modal1' >
        ...
        <div className="PrintButton"><button className='PrintPageButton'onClick={handleShareWhatsApp} >Confirm Order</button></div>
